...
function P_contour(time,R,x_o,y_o,l,w,v_x,v_y,Q,d,s,h,p,r,mask_ex,mask_in,frac_speak,delta_t,delta_x,delta_y){
    ...
    */
    ...
    var temp;
    for (let i=0; i<n_y;i++){
        temp = yImpulse(t,yy_array[i],y_o,l,w,h,v_y,Q);
        for (let j=0;j<n_t;j++){
            I_y[i][j] = temp[j];
        }
    }
    ...
    for (let i=0; i<n_x;i++){
        temp = xImpulse(t,xx_array[i],x_o,l,w,h,v_x,Q);
        for (let j=0;j<n_t;j++){
            I_x[i][j] = temp[j];
        }
    }
    ...
    */
    ...
    for (let i=0;i<n_x;i++){
        for(let j=0;j<n_y;j++){
            ...
            result[j][i] = last_convolve(S,I) / (h/2);
            result[j][i] = round(result[j][i]);
            console.log(`calc C of (${xx_array[i]},${yy_array[j]})`);
        }
    }
...
function Impulse(t,x,y,x_o,y_o,l,w,h,v_x,v_y,Q,d,s){
